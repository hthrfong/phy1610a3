...
float output_time_step(int arrsize, int t, rarray<float,2> &number_of_ants, \
  ...
  if (t==0) {
    std::cout << "Time step" << "\t" << "Ants left" << std::endl; // print out headers at the beginning
  }
  std::cout << t << "\t" << remaining_ants << std::endl; // print out time and number of ants remaining at that time step
